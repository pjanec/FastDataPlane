    ...
    {  
        ...
        private static class UnmanagedAccessor<T> 
        {
            ...
            static UnmanagedAccessor()
            {
                ...
                _register = (RegisterDelegate)Delegate.CreateDelegate(typeof(RegisterDelegate), null, 
                    repoType.GetMethod("RegisterUnmanagedComponent", flags).MakeGenericMethod(typeT));

                _add = (AddUnmanagedDelegate<T>)Delegate.CreateDelegate(typeof(AddUnmanagedDelegate<T>), null, 
                    repoType.GetMethod("SetUnmanagedComponent", flags).MakeGenericMethod(typeT));

                _getRW = (GetRWDelegate<T>)Delegate.CreateDelegate(typeof(GetRWDelegate<T>), null, 
                    repoType.GetMethod("GetUnmanagedComponentRW", flags).MakeGenericMethod(typeT));

                _getRO = (GetROUnmanagedDelegate<T>)Delegate.CreateDelegate(typeof(GetROUnmanagedDelegate<T>), null, 
                    repoType.GetMethod("GetUnmanagedComponentRO", flags).MakeGenericMethod(typeT));

                _has = (HasDelegate)Delegate.CreateDelegate(typeof(HasDelegate), null, 
                    repoType.GetMethod("HasUnmanagedComponent", flags).MakeGenericMethod(typeT));

                _remove = (RemoveDelegate)Delegate.CreateDelegate(typeof(RemoveDelegate), null, 
                    repoType.GetMethod("RemoveUnmanagedComponent", flags).MakeGenericMethod(typeT));
            }
        ...
        private static class ManagedAccessor<T> 
        {
            ...
            static ManagedAccessor()
            {
                ...
                _register = (RegisterDelegate)Delegate.CreateDelegate(typeof(RegisterDelegate), null, 
                    repoType.GetMethod("RegisterManagedComponent", flags).MakeGenericMethod(typeT));

                _add = (AddManagedDelegate<T>)Delegate.CreateDelegate(typeof(AddManagedDelegate<T>), null, 
                    repoType.GetMethod("AddManagedComponent", flags).MakeGenericMethod(typeT));

                _getRW = (GetRWDelegate<T>)Delegate.CreateDelegate(typeof(GetRWDelegate<T>), null, 
                    repoType.GetMethod("GetManagedComponentRW", flags).MakeGenericMethod(typeT));

                _has = (HasDelegate)Delegate.CreateDelegate(typeof(HasDelegate), null, 
                    repoType.GetMethod("HasManagedComponent", flags).MakeGenericMethod(typeT));

                _remove = (RemoveDelegate)Delegate.CreateDelegate(typeof(RemoveDelegate), null, 
                    repoType.GetMethod("RemoveManagedComponent", flags).MakeGenericMethod(typeT));
            }
